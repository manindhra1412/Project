...
let UserController = class UserController {
    constructor(userServerice, jwtService, Handler) {
    ...
    async Signup(response, user) {
        try {
            let param = await (0, parameter_validator_1.validateAsync)(user, ['fullname', 'email', 'mobileNo', 'userType', 'password']);
    ...
    async SignIn(response, user) {
        try {
            let param = await (0, parameter_validator_1.validateAsync)(user, ['mobileNo', 'password']);
